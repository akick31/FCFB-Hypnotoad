...
async def coin_toss_choice_command(client, config_data, discord_messages, coin_toss_choice, message, logger):
    ...
    try:
        ...
        # Format and send kicking number request, first refresh game object
        game_object = await get_ongoing_game_by_channel_id(config_data, message.channel.id, logger)
        ...
        kicking_number_message = discord_messages["kickingNumberMessage"].format(
            kicking_team=game_object["homeTeam"] if receiving_team == game_object["awayTeam"] else game_object["awayTeam"],
            clock=game_object["clock"],
            quarter=game_object["quarter"],
            timer=game_object["gameTimer"])
        await send_direct_message(receiving_coach_discord_object, kicking_number_message, logger)
